...
const CompletedTasksScreen = () => {
  ...
  const { tasks, defaultAutoDeleteAfter } = useSelector((state) => state.tasks);
  ...
  const handleAutoDeleteChange = (value) => {
    dispatch(setAutoDeleteAfterDays(value));
    Alert.alert(
      "Auto-Delete Set",
      `Completed tasks will be deleted after ${value} day(s).`
    );
  };
  ...
  if (completedTasks.length === 0) {
    return (
      <View
        style={[styles.centered, { backgroundColor: currentTheme.background }]}
      >
        <Text style={[styles.noTaskText, { color: currentTheme.text }]}>
          No Completed Task
        </Text>
      </View>
    );
  ...
  return (
    ...
    >
      <View style={styles.autoDeleteContainer}>
        <Text style={[styles.autoDeleteLabel, { color: currentTheme.text }]}>
          Auto Delete After:
        </Text>
        <Picker
          selectedValue={defaultAutoDeleteAfter}
